  ...
  with rocket.HasL1HellaCacheParameters
{
 ...
 // Prioritize emptying the store queue when it is almost full
  ...
  for (w <- 0 until memWidth) {
    ...
    will_fire_ldq_incoming  (w) := lsu_sched(can_fire_ldq_incoming  (w) , false , false , false , false) //    ,    , 
  ...
  for (w <- 0 until memWidth) {
    ...
    when (will_fire_load_incoming(w)) {
      dmem_req(w).valid      := !exe_tlb_miss(w) && !exe_tlb_uncacheable(w) && !(ma_ld(w) || pf_ld(w))
      ...
      dmem_req(w).valid      := !exe_tlb_miss(w) && !exe_tlb_uncacheable(w) && !pf_ld(w)
      dmem_req(w).bits.addr  := exe_tlb_paddr(w)
      dmem_req(w).bits.uop   := exe_tlb_uop(w)
